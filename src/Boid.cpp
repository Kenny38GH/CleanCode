...
void Boid::flock(const std::vector<Boid*> &nearest, const float &dT) {
  ...
  for(auto &n : nearest) {
    ...
    if(length(n->_position - _position) < _view_range/4){
      flee(n->_position);
    }
    else {
      seek(n->_position);
...
bool Boid::sees(const Boid &boid) const {
  ...
  float angle    = abs((vec_dot / (glm::length(_velocity)*glm::length(dist))) * 180/3.141592f);
...
void Boid::update(const float &dT, std::vector<Boid*> nearests) {
